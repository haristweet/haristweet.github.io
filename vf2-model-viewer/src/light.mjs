...
const models=[0,1].map(p=>{ const c=g("sceneChooseModels")(sc,p,cand); return c&&c.map });
...
for(let t=0;t<mesh.count;t+=3){
  ...
  for(let y=y0;y<=y1;y++) for(let x=x0;x<=x1;x++){
    ...
    if(tex){ const lx=w0*V[0][9]+w1*V[1][9]+w2*V[2][9], ly=w0*V[0][10]+w1*V[1][10]+w2*V[2][10];
      const ox=V[0][11],oy=V[0][12],sw=V[0][13],shh=V[0][14],page=V[0][16],pal=V[0][17];
      const X=Math.floor(ox+((lx%sw)+sw)%sw), Y=Math.floor(oy+((ly%shh)+shh)%shh), tx=g("texRam")(col.tex,page,X,Y); tl=col.clut[pal*128+tx*4] }
    rec[i]={dot,c5,tl};
  }
}
...
for(let i=0;i<W*H;i++){ const r=rec[i]; if(!r) continue; const s=i*3, sp=[shot.px[s],shot.px[s+1],shot.px[s+2]];
  ...
  out.push([+r.dot.toFixed(3),r.tl,best[1],best[0],r.c5.join(",")]) }
