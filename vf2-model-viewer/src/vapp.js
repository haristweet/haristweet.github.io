...
function hookInput(){
  ...
  // ホイール: カーソルの下の物が動かないように寄る（寄った倍率 k だけ、中心からのずれを戻す）
...
async function loadStates(files){
...
async function loadArc(f){ if(!f) return;
...
// ページにドロップしたファイルを種類で振り分ける（.p2s＝写し、.zip＝アーケードのロム、それ以外＝ディスク）
function hookDrop(){
  let n=0; const on=v=>document.body.classList.toggle("drop-over",v);
  addEventListener("dragenter",e=>{ e.preventDefault(); n++; on(true) }); addEventListener("dragover",e=>e.preventDefault());
  addEventListener("dragleave",()=>{ if(--n<=0){ n=0; on(false) } });
  addEventListener("drop",async e=>{ e.preventDefault(); n=0; on(false); const fs=[...e.dataTransfer.files];
    const st=fs.filter(f=>/\.p2s$/i.test(f.name)), zip=fs.find(f=>/\.zip$/i.test(f.name)), disc=fs.find(f=>/\.(bin|iso|cvm|img)$/i.test(f.name));
    if(!st.length&&!zip&&!disc){ status("読めるファイルが無い（.p2s・.bin/.iso/.cvm・vf2.zip）",true); return }
    if(disc) await loadDisc(disc); if(zip) await loadArc(zip); if(st.length) await loadStates(st) });
}
// 背景の色（tobal2 のビューアと同じ8種。選んだものは覚えておく）。空を消したとき・「背景なし」でないスクショに出る
...
function setBackground(i){ const p=BG_PRESETS[i]||BG_PRESETS[0], v=$("viewer").style; v.setProperty("--stage2",p.c); v.setProperty("--stage",p.e); APP.bg=p;
  [...$("bg-box").children].forEach((b,k)=>b.setAttribute("aria-pressed",String(k===i))); try{ localStorage.setItem("vf2bg",String(i)) }catch(_){} }
function hookBackground(){
  ...
  let i=0; try{ i=+localStorage.getItem("vf2bg")||0 }catch(_){} setBackground(i);
}
...
const CHARS_ONLY={"c-stage":false,"c-sky":false};
...
function fileBase(){ const b=(APP.mode==="disc"?APP.scene?.names[0]:APP.states[APP.cur]?.name||"vf2").replace(/\.p2s$/i,""), M=APP.mot;
  return b+(M&&M.m&&APP.scene&&APP.scene.sc!==APP.scene.sc0?`_${M.pl?"2P":"1P"}_move${M.m}`:"")+"_v"+VERSION }
...
async function savePng(){
  if(!APP.scene) return; const cv=$("cv"), clear=$("c-clear").checked, crop=$("c-crop").checked;
  let r=null; if(crop) await withView(CHARS_ONLY,()=>{ const b=opaqueBox(null); if(b) r=padBox(b) });
  const blob=await withView(clear?CHARS_ONLY:{},()=>{ const W=cv.width,H=cv.height; r=r||{x:0,y:0,w:W,h:H};
...
function init(){
  ...
  $("b-png").onclick=savePng; hookBackground();
