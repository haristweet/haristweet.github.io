...
# キャラごとの部品の一覧（1色目）と、セーブステートの場面（ゲームの命令の列から組んで写真と重ねた絵）と、ページで撮った顔の一覧
set -e
cd "$(dirname "$0")"
...
# 顔の一覧（ページそのものを Playwright で撮る。python3 assemble.py のあとで）
node faces.mjs out/shots/faces.png >/dev/null
ls out/shots
