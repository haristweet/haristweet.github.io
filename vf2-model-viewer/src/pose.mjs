...
function render(which,stage){
  const mesh=g("sceneMesh")(sc,col,models,{which,stage}), D=mesh.data, S=g("BUILD_STRIDE");
  ...
  for(const V of tris){
    ...
    const dot=V[0][3]*Ln[0]+V[0][4]*Ln[1]+V[0][5]*Ln[2], c5=[V[0][6],V[0][7],V[0][8]], tex=V[0][15]>.5;
    ...
    for(let y=y0;y<=y1;y++) for(let x=x0;x<=x1;x++){
      ...
      if(tex){ const lx=w0*V[0][9]+w1*V[1][9]+w2*V[2][9], ly=w0*V[0][10]+w1*V[1][10]+w2*V[2][10], sw=V[0][13], sh=V[0][14];
        const X=Math.floor(V[0][11]+((lx%sw)+sw)%sw), Y=Math.floor(V[0][12]+((ly%sh)+sh)%sh); const tx=g("texRam")(col.tex,V[0][16],X,Y); if(V[0][15]>1.5&&tx===15) continue; tv=col.clut[V[0][17]*128+tx*4] }
      zb[i]=z; const l=g("buildLuma")(tv,dot,[V[0][19],V[0][20]]);
