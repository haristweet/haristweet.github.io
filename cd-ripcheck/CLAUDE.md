...
| 項目 | 正常音源 | 欠陥入り | 現在の閾値 |
|---|---|---|---|
| `drop_hop_ms`（時間分解能） | — | **音飛びの長さは中央値 2ms** | 2.0 |
| `drop_ratio`（周囲の中央値との比） | 誤検出 0 | 実物の音飛びは中央値 -34.9dB | 0.03（-30dB） |
| `drop_guard_sec`（曲頭の無視） | — | 実物は 1.449s から音飛び | 0.25 |
...
**`drop_hop_ms` を大きくしないこと。** 実ファイルの音飛びは長さの中央値が
2ms しかなく、10ms 刻みでは平均に埋もれて 110 箇所中 1 箇所しか見つからない。
これが最も効いたパラメータ。

**`drop_ratio` を 0.01（-40dB）に戻さないこと。** 実物の音飛びは中央値 -34.9dB
で、-40dB では大半を取り逃す。

**`drop_guard_sec` を 2.0 に戻さないこと。** 実物は 1.449s から音飛びしている。
