    ...
    if(R){
      ...
      if(T1_VRAM&&T1_DECAL_PUSH&&!o.noColor){
        R.faces.forEach((f,i)=>{ const pf=plan?plan.at[i]:null;
          if(!pf||!pf.tex) return;
          const n=f.n||t1FaceNormalRaw(d,o,f.idx);   // 法線の無い面（命令14・15）は形から出す
          const L=n?Math.hypot(n[0],n[1],n[2]):0; if(!(L>1e-6)) return;
          const a=f.idx, sg=fixSeg!=null?fixSeg:((VSEG&&VSEG[a[0]]!=null)?VSEG[a[0]]:f.seg);
          const k=t1PushSign(d,o,a,n,segMid.get(sg|0))/L;
          for(const v of a){ const s=vpush.get(v)||[0,0,0];
            s[0]+=n[0]*k; s[1]+=n[1]*k; s[2]+=n[2]*k; vpush.set(v,s) } });
      ...
      for(const f of R.faces){
        ...
        for(const t of strip){
          ...
          const pc=pf?(j=>(pf.w===0||pf.uvOnly)?(vAt&&vAt[a[j]]!=null?vAt[a[j]]:0)
                        :(pf.w===1||pf.tex)?pf.at
...
// 命令14・15 は法線の無いテクスチャの面（三角12B／四角16B）。マリーの顔の貼りもので見つけた。
// 色の語は1面3語で u,v だけ。色そのものは命令2 の頂点ごとの色を使う
// （顔の部品: 命令13×12=48語 ＋ 命令2の8頂点 ＋ 4面×3語 ＝ 68語 ＝ 272B でぴったり）
                  14:{size:12,n:3,nrm:false},15:{size:16,n:4,nrm:false}};
const T1_SLOTS=256;
...
const T1_COL_WORDS={8:1,9:1,12:4,13:4,14:3,15:3};
const T1_COL_TEX={12:true,13:true,14:true,15:true};        // 語1以降が色ではなく u,v
const T1_COL_UVONLY={14:true,15:true};    // 色の語が無く、語0から u,v（色は頂点ごと）
...
// 面の形から出した法線（ファイルの座標のまま）。向きは t1PushSign で外へそろえる
function t1FaceNormalRaw(d,o,idx){
  const dv=new DataView(d.buffer,d.byteOffset,d.byteLength);
  const P=idx.slice(0,3).map(i=>{ const q=o.base+o.vertPtr+i*8;
    return q+6<=d.length?[dv.getInt16(q,true),dv.getInt16(q+2,true),dv.getInt16(q+4,true)]:null });
...
function t1FaceTex(d,o,pf,corners){
  ...
  // 命令14・15 は色の語が無いので、u,v が1語前から始まる
  const cp=o.base+o.colPtr, q=i=>cp+(pf.at+(pf.uvOnly?-1:0)+i)*4;
...
function t1ColorPlan(run){
  ...
  if(run.colWords!=null&&run.faces.every(f=>f.cW!=null)){
    const at=run.faces.map(f=>({at:f.cAt,w:f.cW,corners:f.idx.length,
                               tex:!!T1_COL_TEX[f.op],uvOnly:!!T1_COL_UVONLY[f.op],vert:f.cW===0}));
  ...
  for(let i=0;i<run.faces.length;i++){
    ...
    at[i]={at:p,w,corners:f.idx.length,tex:!!T1_COL_TEX[f.op],uvOnly:!!T1_COL_UVONLY[f.op]}; p+=w;
  }
  return {at,words:p};
}
// ============================================================
  ...
  while(fp+8<=d.length&&steps<(limit||4096)){
    ...
    if(op in T1_CMD_LEN){
      ...
      for(let f=0;f<cnt;f++){
        ...
        else if(op===14||op===15){ cAt=cw; cW=3; cw+=3 }
